...
- In primo luogo completare la [Guida introduttiva: Inizializzazione delle applicazioni client (C++)](quick-app-initialization-cpp.md) per creare una soluzione Visual Studio iniziale. Questa guida introduttiva "Elencare le etichette di riservatezza" si basa su quella precedente per la creazione corretta della soluzione iniziale.
...
1. Aprire la soluzione di Visual Studio creata nell'articolo precedente "Guida introduttiva: Inizializzazione delle applicazioni client (C++)".
...
4. Verso la fine del corpo `main()`, dopo la parentesi graffa di chiusura `}` del blocco `catch` e prima di `return 0;` (il punto in cui è stata interrotta la guida introduttiva precedente), inserire il codice seguente:
...
Si userà lo script di PowerShell seguente per generare i token di accesso, come richiesto dall'SDK. Lo script usa il cmdlet `Get-ADALToken` dal modulo ADAL.PS installato in precedenza, in "Installazione e configurazione di MIP SDK". 

1. Creare un file di script di PowerShell (con estensione ps1) e copiare e incollare lo script seguente nel file:

   - Aggiornare le variabili `$appId` e `$redirectUri` in base ai valori specificati nella registrazione dell'app in Azure AD. 
...
2. Salvare il file di script per poterlo eseguire in seguito, come richiesto dall'applicazione client.
...
2. Se il progetto viene compilato ed eseguito correttamente, l'applicazione richiede un token di accesso ogni volta che l'SDK chiama il metodo `AcquireOAuth2Token()`. È possibile riutilizzare un token generato in precedenza, se viene richiesto più volte e i valori necessari sono uguali:
...
3. Per fornire una risposta alla richiesta precedente, tornare allo script di PowerShell e:
...
4. Dopo aver fornito il token di accesso, l'output della console dovrebbe mostrare le etichette di riservatezza, in modo simile all'esempio seguente:
