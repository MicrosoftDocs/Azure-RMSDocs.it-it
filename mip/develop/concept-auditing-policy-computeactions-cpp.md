...
description: Questo articolo aiuta a comprendere come usare Microsoft Information Protection SDK per il calcolo.
...
Il calcolo di `mip::Actions` per una nuova etichetta può essere ottenuto usando l'elemento `ExecutionStateImpl` definito in [ExecutionState](concept-auditing-policy-executionstate-cpp.md).
...
* Leggere in che modo si [passano gli eventi di controllo alla pipeline di controllo di Azure Information Protection](concept-auditing-policy-cpp.md)