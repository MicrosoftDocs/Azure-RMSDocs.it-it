...
Use the following information to see what’s new or changed for a supported release. La versione più recente è elencata per prima. 

> [!NOTE]
> Minor fixes are not listed so if you experience a problem with the SDK, we recommend that you check whether it is fixed with the latest GA release. Se il problema persiste, verificare la versione di anteprima corrente.
>  
