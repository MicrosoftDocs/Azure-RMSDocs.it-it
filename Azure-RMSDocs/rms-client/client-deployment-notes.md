...
|                                                                                    **Per abilitare e disabilitare la traccia**                                                                                    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    Aggiornare la chiave seguente del Registro di sistema:<br /><br />HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\MSIPC<br />REG_DWORD: Trace<br /><br />**Value:** 1 per abilitare la traccia, 0 per disabilitare la traccia (impostazione predefinita)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
-   HKEY_LOCAL_MACHINE\Software\Microsoft\MSIPC\TrustedServers\
    ...
    **Value:** se il valore specificato è diverso da zero, il client RMS considera attendibili solo i server specificati che sono configurati nell'elenco TrustedServers e nel servizio Azure Rights Management.
...
-   HKEY_LOCAL_MACHINE\Software\Microsoft\MSIPC\TrustedServers\
    ...
    **Value:** i valori stringa aggiunti in questo percorso della chiave del Registro di sistema possono essere in formato nome di dominio DNS, ad esempio **adrms.contoso.com**, oppure URL completi di server AD RMS attendibili, ad esempio **`https://adrms.contoso.com`** . Se un URL specificato inizia con **https://** , il client RMS usa SSL o TLS per contattare il server AD RMS specificato.
...
1. **Il Registro di sistema di Windows nel computer locale**: Se le impostazioni di individuazione del servizio sono configurate nel registro di sistema, queste impostazioni vengono tentate per prime. 
...
2. **Servizi di dominio di Active Directory**: Un computer aggiunto al dominio esegue una query Active Directory per un punto di connessione del servizio (SCP). 
...
3. **Servizio individuazione di Azure Rights Management**: Il client RMS si connette a **`https://discover.aadrm.com`** , che richiede all'utente di eseguire l'autenticazione.
    ...
    Il formato di *YourTenantURL\> è il seguente: {GUID}. RMS. [Region]. AADRM. com. \<* È possibile trovare questo valore identificando il valore **RightsManagementServiceId** quando si esegue il cmdlet [Get-AipServiceConfiguration](/powershell/module/aipservice/get-aipserviceconfiguration) .
...
2.  Nell'editor del Registro di sistema passare a una delle posizioni seguenti:
    ...
    -   Per la versione a 64 bit di Office su piattaforma x64: HKLM\SOFTWARE\Microsoft\MSIPC\Servicelocation

    -   Per la versione a 32 bit di Office su piattaforma x64: HKLM\SOFTWARE\Wow6432Node\Microsoft\MSIPC\Servicelocation
