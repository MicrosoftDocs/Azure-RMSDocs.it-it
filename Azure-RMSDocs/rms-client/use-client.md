...
- Per la maggior parte degli utenti, si distribuisce il Azure Information Protection client Unified Labeling perché questo client soddisfa le esigenze aziendali per questi utenti. 
    ...
    Per questi utenti, la loro esperienza di etichettatura è molto simile se hanno anche dispositivi che eseguono MacOS, iOS e Android e questi dispositivi hanno una versione di Office che supporta le etichette di riservatezza.
...
- Per un subset di utenti, si distribuisce il client classico perché questi utenti richiedono etichette che applicano la protezione HYOK (Holding your own key).
    ...
    Per questi utenti hanno un'esperienza leggermente diversa se dispongono anche di dispositivi che eseguono MacOS, iOS e Android e che dispongono di una versione di Office che supporta le etichette di riservatezza. Ad esempio, viene visualizzato un **** pulsante Proteggi anziché un pulsante di **riservatezza** sulla barra multifunzione di Office. Per altre differenze, vedere le tabelle seguenti.
...
|Reimpostazione delle impostazioni ed esportazione dei log:| Yes | Sì |
|Autorizzazioni definite dall'utente:| Sì | Sì, con limitazioni: <br /><br />-Solo per Outlook (non inviare): Supportato<br /><br />-Per Word, Excel, PowerPoint ed Esplora file: Supportato quando si configura l'etichetta nella portale di Azure |
...
|Comandi di PowerShell:| Sì | Sì, con limitazioni:<br /><br />- Cmdlet inclusi: [Get-AIPFileStatus](/powershell/module/azureinformationprotection/get-aipfilestatus), [New-AIPCustomPermissions](/powershell/module/azureinformationprotection/New-AIPCustomPermissions), [set-AIPFileClassification](/powershell/module/azureinformationprotection/set-aipfileclassification), [set-AIPFileLabel](/powershell/module/azureinformationprotection/set-aipfilelabel), [set-AIPAuthentication](/powershell/module/azureinformationprotection/set-aipauthentication) <br /><br />-Attualmente non è possibile rimuovere la protezione dai file contenitore (zip,. rar,. 7z,. msg e. pst)|
...
|Ereditarietà delle etichette dagli allegati di posta elettronica:| Sì | Yes  |
|Visualizzazione del pulsante Non inoltrare in Outlook| Sì | No |
...
|Suggerimento per i criteri personalizzabili per le etichette automatiche e consigliate: | Sì <br /><br />Usare il portale di Azure per sostituire il messaggio predefinito con gli utenti | No <br /><br /> Sebbene i centri di amministrazione dispongano di un'opzione per fornire un suggerimento personalizzato, questa opzione non è attualmente supportata dal client Unified Labeling|
|Modificare il livello di protezione predefinito dei file: | Sì <br /><br />È possibile utilizzare le [modifiche del registro di sistema](client-admin-guide-file-types.md#changing-the-default-protection-level-of-files) per eseguire l'override delle impostazioni predefinite di protezione nativa e generica | No |
...
- Autorizzazioni personalizzate come opzione che gli utenti possono selezionare nelle app di Office: Word, Excel e PowerPoint
