...
description: Configurare e gestire i modelli di Rights Management dal portale di Azure.
author: cabailey
ms.author: cabailey
manager: mbaldwin
ms.date: 01/29/2018
...
- I modelli di Rights Management che non sono integrati con le etichette per il tenant vengono visualizzati nella sezione **Protection templates** (Modelli di protezione) dopo le etichette nel pannello **Azure Information Protection - Criteri globali**. È possibile convertire questi modelli in etichette o collegarsi a essi quando si configura la protezione per le etichette. 
...
- I modelli di Rights Management per il tenant vengono visualizzati nel pannello **Azure Information Protection - Global policy**, nella sezione **Protection templates** (Modelli di protezione). Non viene visualizzata alcuna etichetta. Sono visualizzate anche le impostazioni di configurazione specifiche per la classificazione e assegnazione di etichette, ma tali impostazioni non hanno alcun effetto sui modelli o non possono essere configurate. 

## <a name="default-templates"></a>Modelli predefiniti

Quando si ottiene la sottoscrizione ad Azure Information Protection o una sottoscrizione Office 365 che include il servizio Azure Rights Management, vengono creati automaticamente due modelli predefiniti per il tenant. Questi modelli consentono di limitare l'accesso ai soli utenti autorizzati dell'organizzazione. Quando vengono creati, questi modelli dispongono delle autorizzazioni elencate nella documentazione [Configurazione dei diritti di utilizzo per Azure Rights Management](configure-usage-rights.md#rights-included-in-the-default-templates).
...
- **\<nome dell'organizzazione> - Confidenziale** che concede le autorizzazioni di lettura e modifica per il contenuto protetto.
...
I modelli archiviati sono visualizzati come non disponibili nel pannello**Azure Information Protection - Criteri globali**. Non è possibile selezionare questi modelli per le etichette, ma è possibile convertirli in etichette.
...
- I modelli di reparto, ovvero i modelli configurati per un ambito specifico, sono visualizzati nei criteri globali. Attualmente, se si modifica e si salva un modello di reparto, la configurazione dell'ambito viene rimossa. L'equivalente di un modello con ambito nei criteri di Azure Information Protection sono i [criteri con ambito](configure-policy-scope.md). Se si converte il modello in etichetta, è possibile selezionare un ambito esistente.
    ...
    In aggiunta, attualmente non è possibile specificare l'impostazione di compatibilità delle applicazioni per un modello di reparto. Se necessario, è possibile specificare l'impostazione di compatibilità delle applicazioni usando il cmdlet [Set-AadrmTemplateProperty](/powershell/module/aadrm/set-aadrmtemplateproperty) e il parametro *EnableInLegacyApps*.
...
3. Nel pannello **Azure Information Protection - Criteri con ambito** o nel pannello **Criteri:\<nome>** individuare il modello da configurare:
...
2. Nel pannello **Azure Information Protection - Criteri con ambito** o nel pannello **Criteri:\<nome>** fare clic su **Aggiungi una nuova etichetta**.

3. Nel pannello **Etichetta** mantenere il valore predefinito **Abilitato**: **Attivata** per pubblicare questo nuovo modello o modificare l'impostazione in **Disattivata** per creare il modello come modello archiviato. Immettere quindi un nome di etichetta e una descrizione per il nome del modello e la descrizione.

4. Per **Set permissions for documents and emails containing this label** (Impostare le autorizzazioni per i documenti e messaggi di posta elettronica contenenti questa etichetta) selezionare **Proteggi**, quindi selezionare **Protezione**:
...
Per maggiori informazioni sulla configurazione dei criteri di Azure Information Protection, usare i collegamenti nella sezione [Configurazione dei criteri dell'organizzazione](configure-policy.md#configuring-your-organizations-policy).  
