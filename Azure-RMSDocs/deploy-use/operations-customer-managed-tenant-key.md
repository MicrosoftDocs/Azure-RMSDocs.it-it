...
Il processo di reimpostazione della chiave è noto anche come rollover della chiave. Quando si esegue questa operazione, Azure Information Protection interrompe l'uso della chiave del tenant esistente per proteggere documenti e messaggi di posta elettronica e inizia a usare una chiave diversa. I relativi criteri e modelli vengono immediatamente abbandonati, ma questo passaggio è graduale per i client e i servizi esistenti che usano Azure Information Protection. Ciò significa che, per un certo periodo di tempo, parte del nuovo contenuto continua a essere protetta tramite la chiave del tenant precedente.
...
Per reimpostare una chiave, è possibile creare una nuova chiave in Azure Key Vault o usare una chiave diversa già presente in Azure Key Vault. Bisogna quindi seguire le stesse procedure usate per implementare lo scenario BYOK per Azure Information Protection:
...
2. Configurare Azure Information Protection per l'uso della nuova chiave tramite il cmdlet [Use-AadrmKeyVaultKey](/powershell/module/aadrm/use-aadrmkeyvaultkey).
...
Per altre informazioni su ognuno dei passaggi descritti, vedere [Implementazione della chiave del tenant di Azure Information Protection](../plan-design/plan-implement-tenant-key.md#implementing-your-azure-information-protection-tenant-key).
