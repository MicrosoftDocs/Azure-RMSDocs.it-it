...
È possibile eseguire query sulle autorizzazioni di un utente per un documento tramite la funzione [IpcAccessCheck](/previous-versions/windows/desktop/msipc/ipcaccesscheck).
...
|--------------------------|----------------|
|**IPC_GENERIC_ALL** <br><br> Concede tutti i diritti all'utente. <br><br> **Punti di imposizione comuni**: Nessuno |Questo diritto viene usato dal sistema e, in genere, non deve essere selezionato direttamente. <br><br> [IpcAccessCheck](/previous-versions/windows/desktop/msipc/ipcaccesscheck) usa questo diritto per determinare se concedere all'utente altri diritti come in questo esempio.<br><br> `/* fAccessGranted is set to TRUE if either the IPC_GENERIC_WRITE or the IPC_GENERIC_ALL right is granted */` <br><br> `IpcAccessCheck(hKey, IPC_GENERIC_WRITE, &fAccessGranted);`|
...
|Nessun diritto di AD RMS <br><br> Nessuna descrizione <br><br> **Punti di imposizione comuni**: Alt+PrtScn|Chiamare [IpcProtectWindow](/previous-versions/windows/desktop/msipc/ipcprotectwindow) in qualsiasi finestra che esegua il rendering del contenuto del documento.|
...
|**IPC_VIEW_RIGHTS** <br><br> Nessuna descrizione <br><br> **Punti di imposizione comuni**: N/D|Imposti dal sistema. Il sistema non consente allo sviluppatore di eseguire query sull'[**elenco di diritti dell'utente**](/previous-versions/windows/desktop/msipc/ipc-user-rights-list) da una licenza a meno che non venga concesso questo diritto.
